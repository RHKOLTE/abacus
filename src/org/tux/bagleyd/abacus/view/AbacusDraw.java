...
public class AbacusDraw {
	...
	public void drawDecimalPointer(int rail) {
		Graphics g = canvas.getGraphics();

		if (g == null)
			return;
		try {
			drawDecimalPointer(g, rail);
		} finally {
			g.dispose();
		}
	}
...
// - ((model.getSign()) ? 1 : 0);
		...
		drawAllGroupSeparators(g, model.getDecimalPosition(), show);
		if (model.getSign())
				...
				if (deck == AbacusFormat.UP) {
					...
					vDraw.fillRect(g, geo.getOffset().x + 4, yOffset, x - 4, geo.getOffset().y);
						...
						if (model.getSlot() && !anomalyActive) {
							...
							drawDecimalSeparator(g, model.getRails() - model.getDecimalPosition(), show);
							if (anomalyActive) {
								if (model.getAnomalySq() != 0)
									drawAllGroupSeparators(g, model.getDecimalPosition() + model.getAnomalySq(), show);
								else
									drawAllGroupSeparators(g, model.getDecimalPosition() + model.getAnomaly(), show);
							} else {
	...
	void drawDecimalPointer(Graphics g, int rail) {
		g.setColor(geo.getBeadShade(2));
		int x = geo.getMiddleBarPositionX(model.getRails() - rail) + 1;
		int y = geo.getMiddleBarPositionY() + (geo.getMiddleBarHeight() >> 1) - 2;
		if (model.getMode() == AbacusInterface.Modes.Medieval.ordinal()) {
			x += geo.getBeadSize().x / 2;
		}
		vDraw.fillRect(g, x, y, 4, 4);
	}
	...
	void drawAllGroupSeparators(Graphics g, int decimalPosition, boolean show) {
		int separator;

		for (separator = 1; separator <= ((model.getRails() - ((model.getSign()) ? 1 : 0) -
				decimalPosition - 1) / model.getGroupSize());
				separator++)
	...
	void drawAnomaly(Graphics g, int rail, boolean show) {
		...
		vDraw.drawLine(g, x + geo.getRailWidth() + 3, y + 1,
			x, y + ht - 1);
		vDraw.drawLine(g, x + geo.getRailWidth() + 3, y,
			x - 1, y + ht - 1);
		vDraw.drawLine(g, x + geo.getRailWidth() + 4, y + 1,
			x, y + ht);
		vDraw.drawLine(g, x, y + 1,
			x + geo.getRailWidth() + 3, y + ht - 1);
		vDraw.drawLine(g, x - 1, y + 1,
			x + geo.getRailWidth() + 3, y + ht);
		vDraw.drawLine(g, x, y,
			x + geo.getRailWidth() + 4, y + ht - 1);
	}
	...
	void drawCounters(Graphics g, int deck, int rail, int count,
		...
		dy = geo.getMiddleBarY() + geo.getMiddleBarHeight() +
			geo.getDeckHeight(0) / 2 +
			(count - 1) * geo.getBeadSize().y / 2 +
		...
		for (int i = 0; i < count; i++) {
			if (geo.getBeadSize().x < 3) {
				...
				vDraw.fillCircle(g, geo.getBeadSize().x / 2,
					...
					dy + geo.getBeadSize().y + i * geo.getBeadSize().y);
			}
		}
	...
	void drawBead(Graphics g, int deck, int rail, int bead,
		...
		if (show) {
			...
			if ((rail == model.getRails() - 1) && model.getSign()) {
				...
				int offset = 0;
				if (model.checkAnomaly()) {
					if (model.getAnomalySq() != 0)
						offset = model.getAnomalySq();
					else
						offset = model.getAnomaly();
				}
				if ((model.getColorScheme() & AbacusInterface.COLOR_FIRST) != 0 &&
					deck == AbacusFormat.BOTTOM &&
					rail - model.getDecimalPosition() - offset > 0 &&
					(rail - model.getDecimalPosition() - offset) %
