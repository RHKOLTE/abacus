...
#JAVA_HOME=/usr/lib/jvm/java-$(V)-sun
#JAVA_HOME=/usr/lib/jvm/java-$(V)-icedtea
#JAVA_HOME="C:/Program\ Files/Java/jdk$(VER).0"
#JAVA_HOME="/cygdrive/c/Program\ Files/Java/jdk$(VER).0"
#CLASSPATH_OPT=-classpath $(JAVA_HOME)/jre/lib/plugin.jar:.
JAVA_OPT=$(CHECK_OPT) $(COMPLIANCE_OPT) $(CLASSPATH_OPT)
PLUGIN=C:/Program Files/Java/jre$(V)/lib/plugin.jar
...
PACKAGE=$(wIDGET)/Makefile $(wIDGET)/build.xml $(wIDGET)/build.properties\
	...
	$(wIDGET)/tests/*.txt $(wIDGET)/bin/*.sh $(wIDGET)/$(wIDGET)Demo.xml

PACKAGEASCII=Makefile build.xml build.properties\
	*$(H) $(SOURCES) $(ALL_XPMS)

PACKAGEBINARY=$(PNGS) $(AUS) $(JS) $(wIDGET)Demo.xml
...
$(DEST)/$(wIDGET)Demo.xml : $(wIDGET)Demo.xml
	cp $< $(DEST)/$(wIDGET)Demo.xml
...
demo : jar
	if uname | grep -iq cygwin; then\
		java -classpath classes $(PACKAGES)/$(wIDGET)/learn/$(WIDGET)Examples | tr -d '\r';\
	else\
		java -classpath classes $(PACKAGES)/$(wIDGET)/learn/$(WIDGET)Examples;\
	fi
...
sonar :
	...
	echo "sonar.projectVersion=8" >> sonar-project.properties
